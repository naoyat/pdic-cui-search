...
byte* encode_bocu1(unichar* src_codepoint, int src_len, int* dest_size) {
  *dest_size = 0;
  byte* dest = static_cast<byte*>(malloc(4 * src_len + 1));
  ...
  byte *newp = static_cast<byte*>(
      realloc(static_cast<void*>(dest),
              *dest_size + 1));
...
unichar* decode_bocu1(byte* src_bocu1, int src_size, int* dest_size) {
  *dest_size = 0;
  unichar* dest = static_cast<unichar*>(
      malloc(sizeof(unichar) * (src_size + 1)));
  ...
  unichar *newp = static_cast<unichar*>(
      realloc(static_cast<void*>(dest),
              sizeof(unichar) * (*dest_size + 1)));
  return newp ? newp : dest;
}

byte* utf8_to_bocu1(byte* src_utf8, int src_size) {
  if (!src_size) src_size = strlen(reinterpret_cast<char*>(src_utf8));
...
byte* bocu1_to_utf8(byte* src_bocu1, int src_size) {
  if (!src_size) src_size = strlen(reinterpret_cast<char*>(src_bocu1));
...
void bocu1_check(byte* bocu1_encoded_data, int size) {
  ...
  printf("["),
      inline_dump(bocu1_encoded_data, size),
      printf("]");

  int codepoints1_len;
  unichar *codepoints1 =
      decode_bocu1(bocu1_encoded_data, size, &codepoints1_len);
  printf("\n => {"),
      inline_dump16(codepoints1, codepoints1_len),
      printf("} => \n");

  int utf8_len;
  byte *utf8str = encode_utf8(codepoints1, codepoints1_len, &utf8_len);
  printf(" => \"%s\" [", utf8str),
      inline_dump(utf8str, utf8_len),
      printf("]");

  int codepoints2_len;
  unichar *codepoints2 = decode_utf8(utf8str, utf8_len, &codepoints2_len);
  printf(" => {"),
      inline_dump16(codepoints2, codepoints2_len),
      printf("}");

  int bocu1_len;
  byte *bocu1str = encode_bocu1(codepoints2, codepoints2_len, &bocu1_len);
  printf("["),
      inline_dump(bocu1str, bocu1_len),
      printf("]");

  if (codepoints2_len == codepoints1_len &&
      memcmp(reinterpret_cast<char*>(codepoints1),
             reinterpret_cast<char*>(codepoints2),
             sizeof(unichar) * codepoints1_len) == 0) {
    printf(" => ok");
  } else {
    printf(" => NG");
  }
  ...
  if (bocu1_len == size &&
      memcmp(reinterpret_cast<char*>(bocu1str),
             reinterpret_cast<char*>(bocu1_encoded_data),
             size) == 0) {
    printf(" => ok.\n");
  } else {
    printf(" => NG.\n");
  ...
  free(static_cast<void*>(codepoints1));
  free(static_cast<void*>(codepoints2));
  free(static_cast<void*>(bocu1str));
}
