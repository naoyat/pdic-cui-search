...
EnglishEntity* parse_entity(const vector<Word*>& words, unsigned int *ix) {
  ...
  if (surface_ == "don't") {
    ;
  } else if (isupper(surface[0]) && (word->pos_canbe("人名") || word->pos_canbe("地名"))) {
    (*ix)++;
    ent = new EnglishName(word);
  } else if (word->pos_canbe("代名")) {
    (*ix)++;
    ent = new EnglishPronoun(word);
  } else if (surface_ != "in" && surface_ != "like" && (word->pos_canbe("名") || word->pos_canbe("名・形"))) {
    (*ix)++;
...
EnglishModifier* parse_modifier(const vector<Word*>& words, unsigned int *ix) {
  ...
  if (surface_ == "the" || surface_ == "a" || surface_ == "an") {  // a/an => "不" は良いが the => "副" なので
    (*ix)++;
...
EnglishVerb* parse_verb(const vector<Word*>& words, unsigned int *ix) {
  ...
  if (surface_ == "is" || surface_ == "are" || surface_ == "am" || surface_ == "was" || surface_ == "were"
      ...
      || surface_ == "ain't"
      || surface_ == "won't" || surface == "wouldn't" || surface_ == "shouldn't" || surface_ == "couldn't") {
    (*ix)++;
    verb = new EnglishVerb(word);
  } else if (surface_ == "don't" || word->pos_canbe("助動")) {  // don't は { 名, 助動 } なので扱いが厄介
    (*ix)++;
    verb = new EnglishVerb(word);
  } else if (word->pos_canbe("動") || word->pos_canbe("他動") || word->pos_canbe("自動")) {
    (*ix)++;
    verb = new EnglishVerb(word);
  }
...
EnglishVP* parse_vp(const vector<Word*>& words, unsigned int *ix) {
  ...
  if (!verb) {
    // *ix = curr_ix;
    checked_vp[curr_ix] = make_pair(*ix, (EnglishVP *)NULL);
    return NULL;
  }

  EnglishVP *vp = new EnglishVP(verb);
  traverse(adverbs, it) {
    vp->add_adverb(*it);
  }
